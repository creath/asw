...
void ThreadIRCSeed2(void* parg)
{
    ...
    while (!fShutdown)
    {
        ...
        if (fTestNet) {
            Send(hSocket, "JOIN #ASWCoinTEST2\r");
            Send(hSocket, "WHO #ASWCoinTEST2\r");
        } else {
            // randomly join #ASWCoin00-#ASWCoin05
            // int channel_number = GetRandInt(5);

            // Channel number is always 0 for initial release
            int channel_number = 0;
            Send(hSocket, strprintf("JOIN #ASWCoin%02d\r", channel_number).c_str());
            Send(hSocket, strprintf("WHO #ASWCoin%02d\r", channel_number).c_str());
        }
