ASWCoin-qt: Qt4 GUI for ASWCoin
===============================
...
Alternatively, install Qt Creator and open the `ASWCoin-qt.pro` file.

An executable named `ASWCoin-qt` will be built.
...
A warning for people using the *static binary* version of ASWCoin on a Linux/UNIX-ish system (tl;dr: **Berkely DB databases are not forward compatible**).

The static binary version of ASWCoin is linked against libdb5.3.
...
installed causes ASWCoin-qt to crash intermittently.  The issue has been reported as `launchpad bug 857790`_, but
isn't yet fixed.
